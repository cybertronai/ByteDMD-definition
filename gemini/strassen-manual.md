Here is a highly optimized, fully self-contained implementation of Strassen's algorithm with manual memory placement.

To efficiently support Strassen's $O(N^2)$ dynamically allocated intermediate products ($M_1 \dots M_7$) without exploding physical memory, the Allocator hardware model has been upgraded to feature Stack / Arena Semantics. It pushes the bump-pointer recursively and pops it upon return, perfectly bounding peak physical memory capacity tightly to exactly $\approx 3 N^2$.

The script dynamically analyzes and fits the empirical scaling limits (the leading constant and power exponent) by utilizing Log-Log regression and outputs the asymptotic boundaries for you.

## Script (strassen_trace.py)
...
def run_strassen(N: int, T: int = 4, logging=True):
    ...
    def recurse(pA_, sA, pB_, sB, pC_, sC, sz):
        ...
        sp.flush_C()  # Force cache flush to main memory prior to manual matrix assembly 
        def read_M(*indices):
            for i in range(h):
                for j in range(h):
                    for idx in indices:
        ...
        read_M(0, 3, 4, 6) # C11 = M1 + M4 - M5 + M7
        read_M(2, 4)       # C12 = M3 + M5
        read_M(1, 3)       # C21 = M2 + M4
        read_M(0, 1, 2, 5) # C22 = M1 - M2 + M3 + M6
